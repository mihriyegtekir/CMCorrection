...
def main():
    ...
    """
    """
    #Frac Improvement per channel
    np.set_printoptions(threshold=np.inf, linewidth=200, precision=6, suppress=True)

    print("\n=== Fractional Improvement per channel ===")
    for i, val in enumerate(frac_impr):
        print(f"Channel {i+1}: {val}")


    print(f"\nMean Fractional Improvement: {frac_impr_mean}")
    print(f"Mean Coherent Noise Ratio: {coh_ratio_mean}")
    """
...
def plot_coherent_noise(y_true, y_pred, chadc, eventid, nch_per_erx, nerx, inputfolder, plotfolder, label_suffix=""):
    ...
    """
    print("\n=== Coherent Noise Ratio per ERx ===")
    for i, val in enumerate(coh_ratio):
        print(f"ERx {i:02d}: coh_ratio = {val:.4f}")
    """
